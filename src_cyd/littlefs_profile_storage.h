...
// ProfileStore owns the blob byte layout, so this stays dumb: keyed blob CRUD over LittleFS files.
//
// Firmware-only: it #includes <LittleFS.h> (Arduino/ESP-IDF), so — like the NVS settings adapter
// and the LGFX glue — it lives in src_cyd/ and never compiles for the native test targets. The host
// tests drive ProfileStore through FakeProfileStorage instead. LittleFS.begin() is mounted once at
// boot (main.cpp); this adapter creates its own mode directory on first write.
#pragma once
