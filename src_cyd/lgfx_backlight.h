...
//
// Wraps the LGFX object's setBrightness(), which drives the LEDC PWM backlight (configured in
// this board's LGFX header, selected by cyd_board.h). Firmware-only glue in src_cyd/: it pulls in
// the LovyanGFX-backed LGFX type, so it never compiles for the native test targets (host tests
