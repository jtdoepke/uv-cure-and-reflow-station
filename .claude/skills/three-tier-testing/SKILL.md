...
- **New business logic** → `lib/app_logic/`, tested in `test/test_logic_cyd/`. Depends only on
...
- **New UI** → `lib/ui_logic/`, tested in `test/test_ui_cyd/` on LVGL's headless dummy display.
...
- **Never set `test_build_src = yes` in the native envs.** `src_cyd/` is deliberately not
  compiled into test binaries — `src_cyd/main.cpp` includes `LovyanGFX.hpp`, so enabling it
...
- `setUp()`/`tearDown()` run before/after **each** test. In `test_ui_cyd`, `lv_init()` /
...
- `LV_USE_TEST` is `#ifndef`-guarded in `include/lv_conf.h` so `native_ui_cyd` flips it on via
